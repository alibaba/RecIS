...
#include <cuco/detail/flat_hash_map_kernels.cuh>
 ...
 *
 * The `flat_hash_map` supports two types of operations:
 ...
 * for a `flat_hash_map` instance to support `erase`, the user must provide an `erased_key_sentinel`
 * which is distinct from the `empty_key_sentinel` at construction. If `erase` is called on a
 * `flat_hash_map` which was not constructed in this way, a runtime error will be generated.
 *
 ...
 * flat_hash_map<int, int> m{100'000, empty_key_sentinel, empty_value_sentinel,
 * erased_value_sentinel};
 *
...
class flat_hash_map {
 ...
 public:
  ...
  flat_hash_map(flat_hash_map const&) = delete;
  flat_hash_map(flat_hash_map&&)      = delete;

  flat_hash_map& operator=(flat_hash_map const&) = delete;
  flat_hash_map& operator=(flat_hash_map&&)      = delete;
  ...
  /**
   * @brief Constructs a dynamically sized map with the specified number of slots
   * and sentinel values.
   *
   ...
   *
   * @param initial_capacity The total number of slots in the map
   * @param empty_key_sentinel The reserved key value for empty slots
   * @param empty_value_sentinel The reserved mapped value for empty slots
   * @param desired_load_factor The desired load factor for the map
   * @param alloc Allocator used for allocating device storage
   * @param stream Stream used for executing the kernels
   */
  flat_hash_map(std::size_t initial_capacity,
                empty_key<Key> empty_key_sentinel,
                empty_value<Value> empty_value_sentinel,
                double desired_load_factor = 0.60,
  ...
  template <typename InputIt,
            ...
            typename Hash     = cuco::default_hash_function<key_type>,
            typename KeyEqual = cuda::std::equal_to<key_type>>
  void find_and_mask(InputIt first,
                     InputIt last,
                     OutputIt output_begin,
  ...
  template <typename Hash     = cuco::default_hash_function<key_type>,
            typename KeyEqual = cuda::std::equal_to<key_type>>
  void rehash(Hash hash = Hash{}, KeyEqual key_equal = KeyEqual{}, cudaStream_t stream = 0);
 ...
 private:
  class device_view_base {
   protected:
    // Import member type definitions from `flat_hash_map`
 ...
 public:
   ...
   * Example:
   * \code{.cpp}
   * cuco::flat_hash_map<int,int> m{100'000, -1, -1};
   *
  ...
  class device_view : public device_view_base {
   public:
     ...
     *
     *     const map_t::device_view sm_flat_hash_map = device_view.make_copy(g,
     *                                                                    sm_buffer);
     *
     *     for (size_t i = g.thread_rank(); i < number_of_elements; i += g.size())
     *     {
     *         values_found[i] = sm_flat_hash_map.find(keys_to_search[i])->second;
     *     }
     * }
     * @endcode
     *
   ...
   * @return The sentinel value used to represent an empty key slot
   */
  [[nodiscard]] Key empty_key_sentinel() const noexcept { return empty_key_sentinel_; }
   ...
   * @return The sentinel value used to represent an empty value slot
   */
  [[nodiscard]] Value empty_value_sentinel() const noexcept { return empty_value_sentinel_; }
   ...
   */
  [[nodiscard]] Key erased_key_sentinel() const noexcept { return erased_key_sentinel_; }

  /**
   * @brief Constructs a device_view object based on the members of the `flat_hash_map` object.
   *
   * @return A device_view object based on the members of the `flat_hash_map` object
   */
  ...
  /**
   * @brief Constructs a device_mutable_view object based on the members of the `flat_hash_map`
   * object
   *
   * @return A device_mutable_view object based on the members of the `flat_hash_map` object
   */
 ...
 private:
  ...
  double max_load_factor_{};         ///< Max load factor before capacity growth
  Key empty_key_sentinel_{};         ///< Key value that represents an empty slot
  Value empty_value_sentinel_{};     ///< Initial value of empty slot
  Key erased_key_sentinel_{};        ///< Key value that represents an erased slot
  slot_allocator_type allocator_{};  ///< Allocator used to allocate slots
};
}  // namespace cuco

#include <cuco/detail/flat_hash_map.inl>
