...
public:
  ...
  explicit RefCountedPtr(T *ptr) : ptr_(ptr) { Ref(); }

  RefCountedPtr(const RefCountedPtr &rptr) : ptr_(rptr.ptr_) { Ref(); }

  RefCountedPtr(RefCountedPtr &&rptr) : ptr_(rptr.ptr_) { rptr.ptr_ = nullptr; }
  ...
  template <typename... Targs> static RefCountedPtr Create(Targs &&...args) {
    return RefCountedPtr(new T(std::forward<Targs>(args)...), 0);
  }

private:
  RefCountedPtr(T *ptr, int x) : ptr_(ptr) { (void)x; }
  void Ref() {
    if (ptr_ != nullptr) {
      ptr_->Ref();
    }
