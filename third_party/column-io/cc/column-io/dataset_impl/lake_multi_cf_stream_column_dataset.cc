...
#include "column-io/lake/lake_multi_cf_stream_reader.h"
...
const std::string kDatasetName{"LakeMultiCFStreamColumnDataset"};
...
public:
  ...
  MakeIteratorInternal(const std::string &prefix) override {
    return std::shared_ptr<IteratorBase>(new Iterator(
        ...
        absl::StrCat(prefix, "::LakeMultiCFStreamColumnDataset"), ds_name_));
  }
...
private:
  ...
  private:
    void ParseCurrentPointTag() {
        ...
        if (!dataset()->path_.empty()) {
            // E.g. path=pangu://ea119dfssearch7--cn-shanghai/nebula/ai_lake/recommend_gul_rank/gul_cnxh_cvr_live_sample/default_column_family\x1finc1/current/data
            // (cluster)/(project)/(table)/(default_column_family\x1finc_column_family)/(partition)/data
        ...
        point_tag_ = std::make_shared<recis::monitor::PointTag>(std::map<std::string, std::string>{
            ...
            {"ds_name", "lake_multi_cf_stream"} // TODO: add ds_name_ for iterator from constructor
        });
    }
  public:
      ...
      do {
        ...
        if (reader_) {
          *end_of_sequence = false;
          ...
          int64_t curr_sec = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now().time_since_epoch()).count();
          int64_t delay_sec = std::max(0L, curr_sec - reader_->TellTimeStamp() / 1000000);
          metric_cli_->report("read_batch", 1, point_tag_, recis::monitor::PointType::kCounter);
          metric_cli_->report("sample_delay_sec", delay_sec, point_tag_, recis::monitor::PointType::kGauge);
          ...
          // open reader
          Status s = Status::OK();
          reader_.reset(new lake::LakeMultiCFStreamReader());
  ...
  private:
    ...
    std::unique_ptr<lake::LakeMultiCFStreamReader> reader_;
...
Status ReadLakeMultiCFStreamRecordBatch(
    const std::string &path,
  ...
  RETURN_IF_ERROR(LakeMultiCFStreamColumnDatase::ParseConfig(
  ...
  // init reader
  lake::LakeMultiCFStreamReader data_reader;
  auto common_st = lake::Status::OK();
  ...
  if (common_st.code() == lake::Status::Code::kOutOfRange) {
    ...
    return Status::Internal("fail to init lake multi cf stream reader, error: ",
                            common_st.what());
  }
  struct timeval tval;
...
std::tuple<
	...
	std::string
	>
LakeMultiCFStreamColumnDatase::ParseSchema(
  ...
  auto parser = SchemaParser::Make(ReadLakeMultiCFStreamRecordBatch);
...
Status LakeMultiCFStreamColumnDatase::ParseConfig(
...
LakeMultiCFStreamColumnDatase::MakeDataset(const std::string &path,
                                    const std::vector<std::string> &selected_columns,
                                    const std::vector<std::string> &input_columns,
                                    const std::vector<std::string> &hash_features,
                                    const std::vector<std::string> &hash_types,
...
std::shared_ptr<DatasetBuilder> LakeMultiCFStreamColumnDatase::MakeBuilder(
    const std::vector<std::string> &selected_columns,
    const std::vector<std::string> &input_columns,
    const std::vector<std::string> &hash_features,
    const std::vector<std::string> &hash_types,
