...
void FusedSparseAdam(torch::Tensor grad, SparseAdamOptions &options,
  ...
  recis::functional::block_apply_adamw(
      ...
      beta1, beta2, /*weight_decay*/ 1., eps, block_size);
}

std::string kParamNameSep{"sparse_adam_"};
...
SparseAdamOptions::SparseAdamOptions(double lr) : lr_(lr) {}
double SparseAdamOptions::get_lr() const { return lr_; }
...
void SparseAdam::add_parameters(
...
void SparseAdam::InitParamState(const std::string &param_name,
                                HashTablePtr param) {
  ...
  state->exp_avg(param->SlotGroup()->AppendSlot(ExpAvgName(), emb_slot->Dtype(),
                                                emb_slot->FullShape(1)));
  state->exp_avg_sq(param->SlotGroup()->AppendSlot(
      ExpAvgSqName(), emb_slot->Dtype(), emb_slot->FullShape(1)));
